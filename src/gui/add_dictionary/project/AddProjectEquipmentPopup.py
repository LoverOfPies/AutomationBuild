...
class SelectRowButton(Button):
    ...
    def on_press(self):
        equipment = self.owner_class.select().where(self.owner_class.name == self.name_row)
        project = self.dict_class.select().where(self.dict_class.name == self.ui_class.filter_name)
        model_obj = [
            {
                'equipment': equipment,
                'project': project
...
class AddProjectEquipmentPopup(Popup):
    button_obj = ObjectProperty()
