...
from src.db.models.project.ProjectEquipment import ProjectEquipment

from src.gui.BaseUIUtils import init_control_buttons, init_title_layout, init_back_button
from src.gui.add_dictionary.project.AddProjectEquipmentPopup import AddProjectEquipmentPopup
...
class ProjectEquipmentUI:
    screen_name = 'project_equipment_screen'
    parent_screen = 'project_screen'
    table_name = 'Комплектации проекта'
    model_class = ProjectEquipment
    items_list = None
    screen = Screen(name=screen_name)
    add_popup = AddProjectEquipmentPopup
    ...
    def main_layout(self):
        ...
        query = (self.model_class.select(self.model_class.id, Equipment.name)
                 .join_from(self.model_class, Equipment)
                 .join_from(self.model_class, Project)
                 .where(Project.name == self.filter_name)
                 )
        self.items_list = query

        for item in self.items_list:
            data_layout.add_widget(Button(text=item.equipment.name))
        ...
        button_layout = SelectableModalButton(text='Добавить', size_hint=[1, None], height=50, change_flag=True,
                                              ...
                                              owner_class=Equipment, dict_class=Project, ui=self)
