...
class SubgroupModalPopup(Popup):
    ...
    def __init__(self, button_obj, change_flag, dict_class, owner_class, id_value, field, **kwargs):
        super(SubgroupModalPopup, self).__init__(**kwargs)
        self.title = 'Подгруппа'
        self.size = [400, 400]
        ...
        subgroups = Subgroup.select()
        for subgroup in subgroups:
            data_layout.add_widget(Label(text=str(subgroup.id), size_hint_y=None, height=dp(30)))
            data_layout.add_widget(Label(text=str(subgroup.name), size_hint_y=None, height=dp(30)))
            data_layout.add_widget(SelectRowButton(text='Выбрать', height=dp(30), popup=self,
                                                   name_row=subgroup.name, button_obj=button_obj,
