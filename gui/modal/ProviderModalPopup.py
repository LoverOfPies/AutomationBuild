...
class ProviderModalPopup(Popup):
    ...
    def __init__(self, button_obj, change_flag, dict_class, owner_class, id_value, field, **kwargs):
        super(ProviderModalPopup, self).__init__(**kwargs)
        ...
        for provider in providers:
            ...
            data_layout.add_widget(Label(text=str(provider.name), size_hint_y=None, height=dp(30)))
            data_layout.add_widget(SelectRowButton(text='Выбрать', height=dp(30), popup=self,
                                                   name_row=provider.name, button_obj=button_obj,
