...
class TechnologyModalPopup(Popup):
    ...
    def __init__(self, button_obj, change_flag, dict_class, owner_class, id_value, field, **kwargs):
        super(TechnologyModalPopup, self).__init__(**kwargs)
        ...
        for technology in technologies:
            data_layout.add_widget(Label(text=str(technology.id), size_hint_y=None, height=dp(30)))
            data_layout.add_widget(Label(text=str(technology.name), size_hint_y=None, height=dp(30)))
            data_layout.add_widget(SelectRowButton(text='Выбрать', height=dp(30), popup=self,
                                                   name_row=technology.name, button_obj=button_obj,
