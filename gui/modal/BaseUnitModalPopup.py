...
class BaseUnitModalPopup(Popup):
    ...
    def __init__(self, button_obj, change_flag, dict_class, owner_class, id_value, field, **kwargs):
        super(BaseUnitModalPopup, self).__init__(**kwargs)
        self.title = 'Базовые единицы'
        self.size = [400, 400]
        ...
        baseunits = BaseUnit.select()
        for baseunit in baseunits:
            data_layout.add_widget(Label(text=str(baseunit.id), size_hint_y=None, height=dp(30)))
            data_layout.add_widget(Label(text=str(baseunit.name), size_hint_y=None, height=dp(30)))
            data_layout.add_widget(SelectRowButton(text='Выбрать', height=dp(30), popup=self,
                                                   name_row=baseunit.name, button_obj=button_obj,
