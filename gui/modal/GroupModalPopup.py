...
class GroupModalPopup(Popup):
    ...
    def __init__(self, button_obj, change_flag, dict_class, owner_class, id_value, field, **kwargs):
        super(GroupModalPopup, self).__init__(**kwargs)
        self.title = 'Группы'
        self.size = [400, 400]
        ...
        groups = Group.select()
        for group in groups:
            data_layout.add_widget(Label(text=str(group.id), size_hint_y=None, height=dp(30)))
            data_layout.add_widget(Label(text=str(group.name), size_hint_y=None, height=dp(30)))
            data_layout.add_widget(SelectRowButton(text='Выбрать', height=dp(30), popup=self,
                                                   name_row=group.name, button_obj=button_obj,
